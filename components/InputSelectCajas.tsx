...
const InputSelectCajas = ({ cajas, onChange }) => {
  ...
  const [selected, setSelected] = useState<CajaSimple | null>(null);
      ...
      <Combobox value={selected} onChange={handleCajaChange}>
        <div className="relative bg-transparent">
          <div className="relative w-full cursor-default overflow-hidden rounded-lg bg-transparent text-left focus:outline-none focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-opacity-75 focus-visible:ring-offset-2 focus-visible:ring-offset-teal-300 sm:text-sm">
            <Combobox.Input
              className="w-full border-none h-8 py-2 pr-10 leading-5 bg-transparent text-gray-900 focus:ring-0"
              displayValue={(caja: CajaSimple) =>
          ...
          <Transition
            ...
            afterLeave={() => setQuery("")}
          >
            <Combobox.Options className="absolute mt-1 max-h-60 w-full overflow-auto rounded-md bg-amber-300 bg-opacity-1w-20 py-1 text-base ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm">
              {filteredPeople.length === 0 && query !== "" ? (
                <div className="relative cursor-default select-none py-2 px-4 text-gray-700">
                  Nothing found.
                </div>
              ) : (
                filteredPeople.map((caja: CajaSimple) => (
                  <Combobox.Option
                    key={caja.id}
                    className={({ active }) =>
